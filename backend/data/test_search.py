...
def save_to_chromadb(documents: list, chroma_path: str) -> chromadb.Collection:
    ...
    try:
        ...
        print(f"   기존 컬렉션 '{collection_name}' 발견 → 초기화 완료")
    except ValueError:
        pass
    ...
    # 🔴 [여기 수정] ID가 "job_"로 겹치면 뒤에 강제로 숫자를 붙여서 고유하게 만듭니다.
    ids = [
        f"job_{i}" if doc["doc_id"] == "job_" else doc["doc_id"] 
...
def test_search(collection: chromadb.Collection) -> None:
    ...
    for query in test_queries:
        ...
        for i, (doc, meta) in enumerate(
            ...
            print(f"    거리(Distance): {results['distances'][0][i]:.4f}")
            print(f"    문서: {doc[:80]}...")
